<template>
  <div>
    ...
    <div class="flex items-center justify-between mb-8">
      <div>
        <h1 class="text-2xl font-light mb-1">Attendance Records</h1>
        <p class="text-sm text-[#8e8e8e]">Upload and manage event attendance</p>
      </div>
      <button 
    ...
    <div class="border border-ic-primary/20 bg-purple-50 rounded-lg p-6 mb-8">
      <div class="flex items-start gap-4">
        ...
        <div class="flex-1">
          ...
          <ul class="text-sm text-[#8e8e8e] space-y-1">
    ...
    <div>
      ...
      <div class="space-y-3">
        <div 
          ...
          :key="record.id"
          class="border border-[#dbdbdb] rounded-lg p-4"
        >
          <div class="flex items-start justify-between">
            ...
            <div class="flex items-center gap-2">
              <button 
                ...
                class="px-3 py-1.5 text-xs font-medium border border-[#dbdbdb] rounded-lg hover:bg-[#fafafa] transition-colors"
              >
                View Details
              </button>
              <button 
      ...
      <!-- Empty State -->
      <div v-if="attendanceRecords.length === 0" class="text-center py-16 border border-[#dbdbdb] rounded-lg">
        ...
        <p class="text-sm text-[#8e8e8e] mb-4">No attendance records yet</p>
        <button 
          @click="showUploadModal = true"
          class="px-4 py-2 bg-ic-primary text-white text-sm font-semibold rounded-lg hover:bg-ic-secondary transition-colors"
        >
    ...
    >
      <div class="bg-white rounded-xl max-w-md w-full p-6">
        <div class="flex items-center justify-between mb-6">
          ...
          <button @click="closeUploadModal" class="p-1 hover:bg-[#fafafa] rounded-lg">
        ...
        <form @submit.prevent="handleUpload" class="space-y-4">
          <div>
            <label class="block text-sm font-medium text-[#262626] mb-2">Select Event</label>
            <select 
          ...
          <div>
            <label class="block text-sm font-medium text-[#262626] mb-2">Upload File</label>
            <div class="border-2 border-dashed border-[#dbdbdb] rounded-lg p-6 text-center hover:border-ic-primary transition-colors cursor-pointer">
              <input 
                type="file" 
              ...
              <label for="fileInput" class="cursor-pointer">
                <svg class="w-12 h-12 mx-auto text-[#8e8e8e] mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" />
                </svg>
                <p class="text-sm font-medium mb-1">
                  {{ uploadData.fileName || 'Click to upload or drag and drop' }}
                </p>
                <p class="text-xs text-[#8e8e8e]">CSV or Excel file (MAX. 5MB)</p>
              </label>
            </div>
          </div>
